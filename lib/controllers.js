        ...
        store.getObject(
          ...
          (err, object, data) => {
            if (err) {
              ...
              console.error(
                "Custom Error Document not found: " + errorDocument
              );
    ...
    store.getBucket(srcBucket, err => {
      ...
      store.existsObject(srcBucket, srcKey, (err, exists) => {
        ...
        if (!exists) {
          ...
          template = templateBuilder.buildKeyNotFound(srcKey);
          return buildXmlResponse(res, 404, template);
        }
        ...
        store.copyObject(
          ...
          (err, metadata) => {
            ...
            template = templateBuilder.buildCopyObject(metadata);
            return buildXmlResponse(res, 200, template);
          }
        );
      });
  ...
  return {
    ...
    getBucket(req, res) {
      ...
      if (indexDocument) {
        ...
        store.listObjects(req.params.bucket, options, (err, results) => {
          ...
          template = templateBuilder.buildBucketQuery(options, results);
    ...
    postObject(req, res) {
      ...
      if (req.query.uploads !== undefined) {
        ...
        template = templateBuilder.buildInitiateMultipartUploadResult(
          req.params.bucket,
          req.params.key,
          uploadId
        );
          ...
          xml2js.parseString(completeMultipartUploadXml, (err, result) => {
            ...
            store.putObjectMultipart(
              ...
              (err, md5, size) => {
                ...
                const location = `${req.protocol}://${req.headers["host"]}${
                  url.parse(req.originalUrl).pathname
                }`;
                template = templateBuilder.buildCompleteMultipartUploadResult(
                  req.params.bucket,
                  req.params.key,
                  location,
                  md5
