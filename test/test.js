 ...
 */
function resetTmpDir(_fs) {
  return function() {
    try {
      fs.removeSync.call(_fs, tmpDir);
      // eslint-disable-next-line no-empty
    } catch (err) {}
    fs.ensureDirSync.call(_fs, tmpDir);
  };
}
...
// make tests fs-driven
[
  { type: "fs", fs: fs, port: 4569 },
  { type: "memfs", fs: memfs, port: 4570 }
].forEach(function(ctx) {
  describe(util.format("Test: [%s]", ctx.type), function() {
    describe("S3rver Tests", function() {
      ...
      let s3rver;
      let s3Client;
      beforeEach("Reset buckets", function() {
        resetTmpDir(ctx.fs)();
      });
      ...
      it("should trigger a Delete event", function(done) {
        const delSubs = s3rver.s3Event
          .filter(
            eventType =>
              eventType.Records[0].eventName == "ObjectRemoved:Delete"
          )
          .subscribe(event => {
        ...
        s3Client.putObject(putParams, err => {
          ...
          s3Client.deleteObject(
            { Bucket: buckets[0], Key: "testDelete" },
            err => {
              if (err) return done(err);
            }
          );
            ...
            (object, callback) => {
              s3Client.getObject(
                ...
                (err, newObject) => {
                  ...
                  newObject.ContentLength.should.not.equal(
      ...
      it("should not fail to delete a nonexistent object from a bucket", function(done) {
        s3Client.deleteObject(
          { Bucket: buckets[0], Key: "doesnotexist" },
          done
        );
      });
      ...
      it("should list folders in a bucket filtered by a prefix and a delimiter", function(done) {
        const testObjects = [
          ...
          {
            Bucket: buckets[5],
            Key: "folder1/folder2/file3.txt",
            Body: "Hello!"
          },
          {
            Bucket: buckets[5],
            Key: "folder1/folder2/file4.txt",
            Body: "Hello!"
          },
          {
            Bucket: buckets[5],
            Key: "folder1/folder2/file5.txt",
            Body: "Hello!"
          },
          {
            Bucket: buckets[5],
            Key: "folder1/folder2/file6.txt",
            Body: "Hello!"
          },
          {
            Bucket: buckets[5],
            Key: "folder1/folder4/file7.txt",
            Body: "Hello!"
          },
          {
            Bucket: buckets[5],
            Key: "folder1/folder4/file8.txt",
            Body: "Hello!"
          },
          {
            Bucket: buckets[5],
          ...
          {
            Bucket: buckets[5],
            Key: "folder1/folder3/file10.txt",
            Body: "Hello!"
          }
        ];
      ...
      before("Initialize bucket", function(done) {
        const s3rver = new S3rver({
          ...
          s3Client = new AWS.S3({
            accessKeyId: "123",
            secretAccessKey: "abc",
            endpoint: util.format("http://%s:%d", "localhost", ctx.port),
            sslEnabled: false,
            s3ForcePathStyle: true
          });
    ...
    describe("S3rver Tests with Static Web Hosting", function() {
      let s3Client;
      let s3rver;
      beforeEach("Reset site bucket", function() {
        resetTmpDir(ctx.fs)();
      });
        ...
        const s3rver = new S3rver({
          ...
          s3Client.createBucket({ Bucket: bucket }, err => {
            ...
            generateTestObjects(s3Client, bucket, 10, err => {
              ...
              s3rver.close(err => {
                if (err) return done(err);
                const exists = fs.existsSync.call(ctx.fs, directory);
                should(exists).equal(true);
                const files = fs.readdirSync.call(ctx.fs, directory);
